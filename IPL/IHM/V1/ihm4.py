...
class Worker(QObject):
    ...
    def get_y_data1(self):
        return self.y_data1
    
    def get_y_data2(self):
        return self.y_data2
    
    def get_y_data3(self):
        return self.y_data3
    def generate_random_values(self):
        ...
        while True:
            ...
            self.values = [i]+[round(random.random(),2) for _ in range(15)]  # Adjust range as needed
            self.x_data.append(self.values[0])
            self.y_data1.append(self.values[1:10])
            self.y_data2.append(self.values[10:15])
            self.y_data3.append(self.values[-1])
            i+=1
            self.update_signal.emit(self.values)
    ...
    def update_plot(self,frame):
        ...
        for i, checkbox in enumerate(checked_boxes):
            if num_checked_boxes == 1 and checkbox.isChecked():
                ...
                self.ax.plot(self.worker.get_x_data(), [data[i] for data in self.worker.get_y_data1()], label=f'Curve {i+1}')
            elif checkbox.isChecked():
                checkbox.setEnabled(True)
                self.ax.plot(self.worker.get_x_data(), [data[i] for data in self.worker.get_y_data1()], label=f'Curve {i+1}')

        if x_length == -1:
            self.ax.set_xlim(min(self.worker.get_x_data()), len(self.worker.get_x_data())-1)
        elif len(self.worker.get_x_data())-1 - x_length < 0:
            self.ax.set_xlim(0, x_length)
        else:
            self.ax.set_xlim(len(self.worker.get_x_data())-1 - x_length, len(self.worker.get_x_data())-1)
...
class RealTimePlot2(QMainWindow):
    ...
    def update_plot(self,frame):
        ...
        for i, checkbox in enumerate(checked_boxes):
            if num_checked_boxes == 1 and checkbox.isChecked():
                checkbox.setEnabled(False)
                self.ax.plot(self.worker.get_x_data(), [data[i] for data in self.worker.get_y_data2()], label=f'Curve {i+1}')
            elif checkbox.isChecked():
                checkbox.setEnabled(True)
                self.ax.plot(self.worker.get_x_data(), [data[i] for data in self.worker.get_y_data2()], label=f'Curve {i+1}')
...
class RealTimePlot3(QMainWindow):
    ...
    def update_plot(self,frame):
        ...
        for i, checkbox in enumerate(checked_boxes):
            if num_checked_boxes == 1 and checkbox.isChecked():
                checkbox.setEnabled(False)
                self.ax.plot(self.worker.get_x_data(), self.worker.get_y_data3(), label=f'Curve {i+1}')
            elif checkbox.isChecked():
                checkbox.setEnabled(True)
                self.ax.plot(self.worker.get_x_data(), self.worker.get_y_data3(), label=f'Curve {i+1}')
