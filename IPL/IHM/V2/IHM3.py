...
class Worker(QObject):
    ...
    def write_csv_arduino(self):
        ...
        if arduino_test!=0:
            ...
            client_socket.sendto(bytearray([0xDD,0xDD]), address)  # Launch the Arduino
        ...
        while True:
            ...
            if arduino_test == 0:
                ...
                values = [i] + [round(100*np.sin(2*np.pi*0.02*i + 3*k/2),1) for k in range(16)]
                ...
                if phase=='ON':
                    with open('data_1kHz.csv', 'a', newline='') as csvfile:
                        writer = csv.writer(csvfile)
                        writer.writerow(values)
                    k+=1
                i += 1
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.frames = [QFrame(self.centralwidget) for _ in range(15)]
        self.dims_frame=[(600, 620, 111, 91),(770, 540, 111, 91),(600, 70, 111, 91),
              ...
              (220, 780, 111, 70),(300, 120, 111, 70),(300, 620, 111, 70)
              ]
        ...
        self.pressurize=[QPushButton(self.centralwidget) for _ in range(3)]   # Button to open valve
        self.dim_pressurize=[(322, 155, 70, 28),(322, 655 , 70, 28),(242,815, 70, 28)]
        self.spin_pressure=[QSpinBox(self.centralwidget) for _ in range(3)]
        self.dim_spinbox_press=[(322, 125, 71, 28),(322, 625 , 71, 28),(242,785, 71, 28)]
        for pressurize,dim_pressurize,spin_pressure,dim_spinbox_press in zip(self.pressurize,self.dim_pressurize,self.spin_pressure,self.dim_spinbox_press):
            x,y,h,w=dim_pressurize
            pressurize.setGeometry(QRect(x,y,h,w))
            font = QFont("Arial", 7, QFont.Bold)
            pressurize.setFont(font)

            # Set position and dimension of the spinboxes
            x,y,h,w,= dim_spinbox_press
            spin_pressure.setGeometry(QRect(x,y,h,w))
            spin_pressure.setMinimum(1)   # Set minimum value of the spinbox
            spin_pressure.setValue(16)    # Set default value of the spinbox
            spin_pressure.setMaximum(20) # Set maximum value of the spinbox (put N to get N as a max value)    
            if dim_spinbox_press==(242,785, 71, 28):
                spin_pressure.setMinimum(1)   # Set minimum value of the spinbox
                spin_pressure.setValue(4)    # Set default value of the spinbox
                spin_pressure.setMaximum(10) # Set maximum value of the spinbox (put N to get N as a max value) 
            spin_pressure.setStyleSheet("background-color: white;")
            #spinbox.valueChanged.connect(functools.partial(self.spinboxval,idd)) ##################################################################################
            spin_pressure.raise_()
            # Display the objects
            pressurize.raise_()
    ...
    def retranslateUi(self, MainWindow):
        ...
        for pressurize in self.pressurize:
            pressurize.setText(_translate("MainWindow", "Pressurize"))
            pressurize.setStyleSheet("""
            QPushButton {
                border: 1px solid black;
                background-color: white;
                border-radius: 3px;
    ...
    def control_actuator(self,value1,value2):
        ...
        if arduino_test==0:
            ...
            try:
                ...
                self.client_socket.sendto(bytearray([0xF0,value1,0xF1,value2]), address)
                rec_data, addr = self.client_socket.recvfrom(1024)
                if rec_data==bytearray([0xEE, 0xEE, 0xEE, 0xEE, value1,value2]):
                    self.position_sent = QLabel(self.centralwidget)
    ...
    def close_all_program(self):
        ...
        if reply == QMessageBox.Yes:# define server IP and port
            client_socket = socket(AF_INET, SOCK_DGRAM)
            client_socket.settimeout(1)
            if arduino_test!=0:
                client_socket.sendto(bytearray([0xCC,0xCC]), address)  # Launch the Arduino
            with open("data_1kHz.csv", "r") as file:
                ...
                for row in csv_reader:
                    ...
                    y_data.append([float(row[i]) for i in range(1, 17)])
            for i in range(0,10):
                plt.plot(x_data,[data[i] for data in y_data])
            plt.savefig("Pressure at 1kHz.png")
            for i in range(10,15):
                plt.plot(x_data,[data[i] for data in y_data])
            plt.savefig("Temperature at 1kHz.png")
            plt.plot(x_data,[data[15] for data in y_data])
            plt.savefig("Force at 1kHz.png")
    ...
    def send_command(self,command):
        self.client_socket = socket(AF_INET, SOCK_DGRAM)
        self.client_socket.settimeout(1)
        self.client_socket.sendto(command, address)
        rec_data, addr = self.client_socket.recvfrom(1024)
        return rec_data
    ...
    def open_valve(self,valve_number, valve_label,valve_status):
        idds=[11,12,21,22,31,32,33,34,35,51,61,62]
        barray=[bytearray([0x00,1]),bytearray([0x01,1]),bytearray([0x02,1]),
                bytearray([0x03,1]),bytearray([0x04,1]),bytearray([0x05,1]),
                bytearray([0x06,1]),bytearray([0x07,1]),bytearray([0x08,1]),
                bytearray([0x09,1]),bytearray([0x0A,1]),bytearray([0x0B,1])]
        for array,idd in zip(barray,idds):
            if idd==valve_number:
                command=array
                print(idd)
                print(command[0])
        if arduino_test==0:
            ...
            try:
                ...
                print("send")
                if rec_data == bytearray([0xEE, 0xEE, 0xEE, 0xEE,command[0],command[1]]):
                    self.update_valve_status(valve_label, "Open", Qt.darkGreen)
                    self.update_valve_status(valve_status, "Open", Qt.darkGreen)
            except:
                pass
    ...
    def close_valve(self,valve_number, valve_label,valve_status):
        idds=[11,12,21,22,31,32,33,34,35,51,61,62]
        barray=[bytearray([0x00,0]),bytearray([0x01,0]),bytearray([0x02,0]),
                bytearray([0x03,0]),bytearray([0x04,0]),bytearray([0x05,0]),
                bytearray([0x06,0]),bytearray([0x07,0]),bytearray([0x08,0]),
                bytearray([0x09,0]),bytearray([0x0A,0]),bytearray([0x0B,0])]
        for array,idd in zip(barray,idds):
            if idd==valve_number:
                command=array
        if arduino_test==0:
...
####################################################################    
    ...
    def view_json(self): 
        ...
        global json_data
        json_data=[value for key, value in self.data_json[self.comboBox.currentText()].items()]
        for value,l,dim,name,unit in zip(json_data,self.label_preselect[7:],self.dim_preselect,names,units):
            x,y,w,h=dim
...
class CountdownWidget(QWidget):
        ...
        if self.count_sec > 0 or self.count_msec > 0:
            ...
            try:
                if json_data[0]=='---':
                    ...
                    try:
                        if arduino_test==0:
                            pattern_str=['NONE','SQUARE','CIRCLE','+','-']
                            i=0
                            for element in pattern_str:
                                if element==json_data[4]:
                                    pattern=i
                                else:
                                    i+=1
                            time_test = (1000*int(json_data[0])).to_bytes(2, byteorder='big', signed=True)
                            test_profil = bytearray(bytes([0xFF])+
                                                    time_test + 
                                                    bytes([int(json_data[1])]) + 
                                                    bytes([int(json_data[2])]) + 
                                                    bytes([int(json_data[3])]) + 
                                                    bytes([int(pattern)]) + 
                                                    bytes([int(json_data[5]) & 0xFF]))
                            print(test_profil)
                            global phase
                            phase='ON'
                            ...
                            test_profil = bytearray(bytes([0xFF])+
                                                    ...
                                                    bytes([int(pattern)]) + 
                                                    bytes([int(json_data[5]) & 0xFF]))
                            client_socket.sendto(test_profil, address)
                            data, addr = client_socket.recvfrom(4096)  # Read response from arduino
                            if data==bytearray([0xEE, 0xEE, 0xEE, 0xEE, 0xEE, 0xEE]):
                                pass
                    except:
                        pass
            except NameError:
    ...
    def warning(self,error):
        error_message=['No profil test selected',
                       '']
        msgBox = QMessageBox()
        msgBox.setIcon(QMessageBox.Warning)   
        reply = msgBox.warning(MainWindow, "Warning", 
            f'{error_message[error]}', QMessageBox.Ok)
        
    def close(self):
        self.timer.stop()  # Stop the timer if running
        super().close()    # Close the widget
