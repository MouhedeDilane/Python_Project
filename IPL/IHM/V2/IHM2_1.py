        ...
        while True:
            ...
            if self.arduino == 0:
                ...
                try:
                    ...
                    pass
            ############################################################# Worker always at 1kHz??? (no signal needed for switch, freq depending only on arduino)
            # Ensure synchronisation at the desired frequency
            # t1 = time.perf_counter()
            # elapsed_time = t1 - t0
            # # print(elapsed_time)
            # if elapsed_time < 1/self.freq:
...
class Ui_MainWindow(object):
    ...
    def control_actuator(self,value1,value2):
        ...
        if self.arduino==0:
            ...
            try:
                ...
                self.client_socket.sendto(bytearray([0xF0,value1,value2]), address)
                rec_data, addr = self.client_socket.recvfrom(1024)
                if rec_data==bytearray([0xEE, 0xEE, 0xEE, 0xEE, 0xF0, value1,value2]):
                    self.position_sent = QLabel(self.centralwidget)
    ...
    def close_all_program(self):
        ...
        if reply == QMessageBox.Yes:
            address = (self.adress, 5000)  # define server IP and port
            client_socket = socket(AF_INET, SOCK_DGRAM)
            client_socket.settimeout(1)
